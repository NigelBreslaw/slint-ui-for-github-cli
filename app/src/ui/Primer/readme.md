...
- **LayoutTokens** — control sizes, padding, typography lengths, border radius (no light/dark literals). Banner spacing and icon metrics: `banner-padding-default`, `banner-padding-compact`, `banner-icon-size-default`, `banner-icon-size-title-hidden`, `banner-icon-size-inline` (16px for **`warning`** leading icon), `banner-icon-container-padding`, `base-text-weight-semibold`, `banner-actions-gap` (12px, matches `Banner.module.css` action `column-gap`), `banner-dismiss-margin-when-actions` (2px when actions + dismiss).
...
| `description`             | `string`        | Body copy when **`review-request-notice`** is false.                                                                                                                                                                                                              |
| `review-request-notice`   | `bool`          | Default **`false`**. When **`true`**, shows the GitHub-style review sentence with bold login via **`StyledText`** / `@markdown` (only **`review-request-login`** is interpolated). Empty login → “Someone requested…”.                                         |
| `review-request-login`    | `string`        | GitHub login for the bold segment when **`review-request-notice`** is true.                                                                                                                                                                                       |
| `detail`                  | `string`        | Optional muted second line (12px); empty hides it.                                                                                                                                                                                                                |

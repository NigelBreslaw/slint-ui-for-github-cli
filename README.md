...
When signed in, the app also dumps **project-related** data for debug: `projects-v2--orgs-membership.json` for org membership, then REST **Projects V2** and **`gh project list`** for the **`slint-ui`** org only (`projects-v2--org--slint-ui…`, `projects-gh-cli--org--slint-ui.json`). It does **not** write user-scoped project files (`projects-v2--user--…`, `projects-gh-cli--user.json`) in that mode. The debug helper without an org filter also dumps **your** user projects and **all** of your orgs (for local experimentation). V2 and `gh project list` need the **`read:project`** scope (see [scopes.md](scopes.md)). Failures appear as `*--error.json` for that call.

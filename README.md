...
When signed in, the app also dumps **project-related** data for debug: REST **Projects V2** (`projects-v2--…`), output of **`gh project list`** for you and each org (`projects-gh-cli--user.json`, `projects-gh-cli--org--….json`), plus `projects-v2--orgs-membership.json` for org membership. V2 and `gh project list` need the **`read:project`** scope (see [scopes.md](scopes.md)). Failures appear as `*--error.json` for that call.

...
Use this note when implementing Primer-style **anchored** menus or panels (e.g. **SelectPanel** as a dropdown, upstream Primer **AnchoredOverlay**). **`packages/primer-slint`** exports **`AnchoredOverlay`** ([`anchored-overlay.slint`](../../../primer-slint/Overlays/anchored-overlay.slint)) — a **`PopupWindow`** shell with **`OverlayPanelChrome`**, outside-* placement, align/offsets, and optional backdrop — plus this coordinate recipe for custom hosts. The **standalone gallery** also has [`gallery-anchored-overlay-page.slint`](gallery-anchored-overlay-page.slint) and the **SelectPanel** playground ([`gallery-select-panel-page.slint`](gallery-select-panel-page.slint)) — **`SelectPanel`** composes **`AnchoredOverlay`** + **`FilteredActionList`** (built-in trigger by default; see **External anchor** below).
...
**Panel body:** [`FilteredActionList`](../../../primer-slint/FilteredActionList/filtered-action-list.slint) is the filter + list host; [`SelectPanel`](../../../primer-slint/SelectPanel/select-panel.slint) adds title, subtitle, optional footer, and (by default) a trigger **`Button`** + **`AnchoredOverlay`**. For **inline** surfaces (modal dialog, custom trigger), set **`has-built-in-anchor: false`**, bind **`open`**, and pass **`anchor-position-props`** from your trigger’s **`absolute-position`** / size — see gallery **SelectPanel → External anchor** (~line 448 in [`gallery-select-panel-page.slint`](gallery-select-panel-page.slint)). App examples: project board import and time-reporting picker use **`FilteredActionList`** inside **`ModalOverlay`**. Tokens: **`OverlayTokens`** (**`backdrop-scrim`** is for modals such as [`primer-dialog.slint`](../../../../app/src/ui/components/primer-dialog.slint)).

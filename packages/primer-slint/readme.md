...
- **DialogTokens** — Primer **`Dialog`** region padding from **`Dialog.module.css`** (mapped to **primer-tokens** `base-size-*`): **`header-padding`**, **`header-content-padding-inline`**, **`header-content-padding-block`**, **`body-padding`**, **`footer-padding`**, **`footer-actions-gap`**; **`overlay-enter-duration`** composes **`LayoutTokens.duration-200`** for modal enter motion. See audit comment in [`tokens.slint`](tokens.slint).

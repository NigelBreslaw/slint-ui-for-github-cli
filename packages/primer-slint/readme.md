...
- **OverlayTokens** — **`backdrop-scrim`** for modal-style dimmers; **`panel-background`** / **`panel-border`** / **`panel-border-width`** / **`panel-border-radius`**, **`panel-elevation-shadow`** (`shadow.floating.small` via **ShadowTokens**) for floating panels (**AnchoredOverlay** and **ModalOverlay** use the panel tokens; **ModalOverlay** also uses **`backdrop-scrim`**). **No literals** — composes **PrimerColors**, **LayoutTokens**, **ShadowTokens**.
- **DialogTokens** — Primer **`Dialog`** region padding from **`Dialog.module.css`** (mapped to **primer-tokens** `base-size-*`): **`header-padding`**, **`header-content-padding-inline`**, **`header-content-padding-block`**, **`body-padding`**, **`footer-padding`**, **`footer-actions-gap`**. **Only** **LayoutTokens** — see audit comment in [`tokens.slint`](tokens.slint).
...
**Imports for views:** [`primer.slint`](primer.slint) — **`AnchoredOverlay`**, **`AnchoredOverlaySide`**, **`AnchoredOverlayAlign`**, **`ModalOverlay`**, **`OverlayPanelChrome`**, **`SelectPanel`**, **`SelectPanelMode`**, **`SelectPanelItem`**, **`Select`**, **`SelectOption`**, **`ValidationStatus`**, **`DialogTokens`** (optional **`OverlayTokens`** for overrides).

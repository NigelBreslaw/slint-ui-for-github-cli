...
- **LayoutTokens** — control sizes, padding, typography lengths, border radius (no light/dark literals). **ActionList** large row density: `action-list-item-min-height-large`, `action-list-item-padding-block-large`. **ActionList** item divider vertical spacing matches [`ActionList.module.css`](https://github.com/primer/react/blob/main/packages/react/src/ActionList/ActionList.module.css) `.Divider`: `action-list-item-divider-margin-block-start` (= `--base-size-8` − `--borderWidth-thin` via `button-content-gap` − `table-border-width`), `action-list-item-divider-margin-block-end` (= `--base-size-8` / `button-content-gap`). Banner spacing and icon metrics: `banner-padding-default`, `banner-padding-compact`, `banner-icon-size-default` (20px leading icon, matches `Banner.module.css` `.BannerIcon svg`), `banner-icon-size-title-hidden` (16px when `hide-title` and no actions), `banner-icon-container-padding`, `base-text-weight-semibold`, `banner-actions-gap` (12px, matches `Banner.module.css` action `column-gap`), `banner-dismiss-margin-when-actions` (2px when actions + dismiss). **Label** metrics: `label-height-small` / `label-height-large`, `label-padding-inline-*`, `label-border-width`, `label-border-radius-pill`. **LabelGroup** row gap: `label-group-gap` (`LabelGroup.module.css`); item row min-height reuses `control-small-size` (28px). **DataTable** / `Table.module.css`: `table-cell-padding-block-*` / `table-cell-padding-inline-*` (condensed | normal | spacious), `table-cell-edge-inset-inline`, `table-border-radius`, `table-border-width`.
...
**ActionList** ([`ActionList/action-list.slint`](ActionList/action-list.slint)) is a vertical list shell aligned with Primer [**ActionList**](https://primer.style/product/components/action-list/). **`[ActionListLine]`** entries: **`ActionListLineKind.row`** (delegates to **`ActionListRow`** with **`ActionListRowVariant`**, block vs **`ActionListDescriptionLayout`** inline description, **`large`**, trailing text, leading icon), **`divider`** (item hairline), **`section_divider`** (muted full-width rule), **`section_heading`** (**`ActionListSectionHeadingVariant`** subtle | filled). Row-only fields: **`ActionListListboxMode`** (**`listbox-mode`**, **`listbox-focused`**, **`checked`** for the multi-select lead cell (Primer **`.MultiSelectCheckbox`**-style affordance, not the **`Checkbox`** component), **`label-link-style`**, **`has-leading-avatar`** + **`avatar-source`**, **`has-trailing-action`** + **`trailing-action-icon`**. **`listbox-focus-bar-outside`** draws the single-mode focus accent beside the row so **`ActionListRow`** does not duplicate it (**`show-listbox-focus-bar`** stays false for those lines while focused). **`item-activated(index)`** skips **inactive**, **loading**, and **`disabled`** rows; **`trailing-action-clicked(index)`** fires for the trailing **IconButton**. **`ActionListTokens`** supply danger / inactive / link / heading / divider colors. **`LayoutTokens`** includes **`action-list-avatar-diameter`** (and **`-large`**) for list density.

**Imports for views:** [`primer.slint`](primer.slint) — **`ActionList`**, **`ActionListLine`**, **`ActionListLineKind`**, **`ActionListListboxMode`**, **`ActionListRowVariant`**, **`ActionListDescriptionLayout`**, **`ActionListSectionHeadingVariant`**, **`ActionListRow`**, **`ActionListTokens`**, **`LayoutTokens`** (large row + avatar metrics when needed).
...
**Imports for views:** [`primer.slint`](primer.slint) — **`AnchoredOverlay`**, **`AnchoredOverlaySide`**, **`AnchoredOverlayAlign`**, **`SelectPanel`**, **`SelectPanelMode`**, **`SelectPanelItem`**, **`Select`**, **`SelectOption`**, **`ValidationStatus`** (optional **`OverlayTokens`** for dialog chrome).

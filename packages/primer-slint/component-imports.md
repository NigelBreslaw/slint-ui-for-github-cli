...
- **AnchoredOverlay** (anchored `PopupWindow` shell): [`AnchoredOverlay/anchored-overlay.slint`](AnchoredOverlay/anchored-overlay.slint) — **`OverlayTokens`** for panel chrome and elevation (not the modal **`backdrop-scrim`**); `LayoutTokens` for body padding. Pass **`window-inner-height`** from the root **`Window`** for flip math, bind **`anchor-*`** from the trigger’s **`absolute-position`** / size; see [`gallery/anchored-popupwindow.md`](gallery/anchored-popupwindow.md).

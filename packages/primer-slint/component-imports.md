...
- **AnchoredOverlay** (anchored `PopupWindow` shell): [`AnchoredOverlay/anchored-overlay.slint`](AnchoredOverlay/anchored-overlay.slint) — `PrimerColors` + `LayoutTokens` + `ShadowTokens` (internal); pass `window-inner-height` from the root `Window`, bind `anchor-*` from the trigger’s `absolute-position` / size; see [`gallery/anchored-popupwindow.md`](gallery/anchored-popupwindow.md).

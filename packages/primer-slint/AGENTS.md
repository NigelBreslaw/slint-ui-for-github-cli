...
**Standalone gallery:** From the monorepo root, `pnpm dev:gallery` runs **`gallery-main.ts`**, which loads **`gallery/gallery-window.slint`** (header + sidebar; main area switches by group). **Buttons**, **Navs**, **Feedback**, **Forms**, and **Data** are implemented under **`gallery/`** (`gallery-*-page.slint`, ported from `app/src/ui/views/primer-gallery.slint`). Uses **`slint-bridge-kit`** for `GalleryState` wiring.

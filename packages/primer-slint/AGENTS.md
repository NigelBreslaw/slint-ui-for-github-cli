...
**Location:** Library source lives in **`packages/primer-slint/slint/`**. The **github-app** package imports it with **relative paths** from each `.slint` file (for example `../../../packages/primer-slint/slint/primer.slint` from `app/src/ui/main.slint`, or `../../../../packages/primer-slint/slint/...` from files under `app/src/ui/components/` or `app/src/bridges/slint/`).

**Standalone gallery:** From the monorepo root, `pnpm dev:gallery` runs **`packages/slint-gallery/node/gallery-main.ts`**, which loads **`packages/slint-gallery/ui/gallery-window.slint`** (header + sidebar; main area switches by group). **Buttons**, **Navs**, **Feedback**, **Forms**, **Data**, and **Action list** are under **`packages/slint-gallery/ui/views/`** (`gallery-*-page.slint`). Uses **`slint-bridge-kit`** for `GalleryState` wiring. **`AnchoredOverlay`** / anchored geometry with **`PopupWindow`**: [`anchored-popupwindow.md`](../slint-gallery/ui/views/anchored-popupwindow.md).

## Purpose

- **Audience:** Humans and AI assistants adding **Primer-style** UI in Slint under **`packages/primer-slint/slint/`** (consumed by github-app via relative imports).
...
- **Barrel:** [`primer.slint`](slint/primer.slint) re-exports components, `LayoutTokens`, `PrimerColors`, `ButtonTokens`, `CheckboxTokens`, **`ToggleSwitch`**, **`ToggleSwitchTokens`**, `TextInputTokens`, `BannerTokens`, `LabelTokens`, **`ActionListTokens`**, **`Dialog`**, **`DialogHeader`**, **`DialogBody`**, **`DialogFooter`**, **`DialogWidthPreset`**, **`DialogHeightPreset`**, **`DialogAlignPreset`**, **`DialogPositionPreset`**, **`DialogNarrowPositionPreset`**, **`DialogFooterButtonLayout`**, **`DialogFooterAutoFocus`**, **`Icons`**, `Size`, and shared types (**DataTable**, **Select**, **SelectPanel**, **Label**, etc.). When adding exports or model fields, update [`readme.md`](readme.md) where noted (e.g. DataTable **Imports for views**).
- **Tokens:** [`tokens.slint`](slint/tokens.slint) — several `export global` singletons in **one file**. **Order matters:** `PrimerColors` → `ButtonTokens` → `CheckboxTokens` (uses `ButtonTokens`) → **`ToggleSwitchTokens`** (uses `PrimerColors` + `CheckboxTokens` `out` only) → `TextInputTokens` (uses `PrimerColors` + `ButtonTokens`) → `BannerTokens` → `LabelTokens` → **`ActionListTokens`** (uses `PrimerColors` + `ButtonTokens` `out` only) (`BannerTokens` / `LabelTokens` / `ActionListTokens`: no literals in those globals).

### Token layers (current convention)

| Global | Contents |
| ------ | -------- |
| **LayoutTokens** | Lengths, typography, motion **`duration`** steps (**`duration-80`** ToggleSwitch, **`duration-200`**, **`duration-250`**, …), control dimensions, padding, radii, icon sizes, banner spacing. **No** light/dark colors. |
...
| **DialogTokens** | Primer **`Dialog`** spacing + typography (**`LayoutTokens`**), **`width-*`** (**React **`widthMap`** literals vs **`overlay.width`** elsewhere), **`overlay-enter-duration`** / **`overlay-sheet-enter-duration`**. Audit table in [`tokens.slint`](slint/tokens.slint). |

**Cross-global rule:** Dependents use other globals’ **`out`** bindings only — not their **private** fields.

### Icons (`assets/icons.slint`)

Single registry for bundled SVGs — see [`assets/icons.slint`](slint/assets/icons.slint). **Full** naming rules, checklist for new icons, Banner usage, and TS bridge: [`primer-slint-icons-registry`](../../.cursor/skills/primer-slint-icons-registry/SKILL.md) skill.
...
2. Add `packages/primer-slint/slint/<Name>/` with a clear root `*.slint` (subfolders if needed).
3. **Imports:** see [**component-imports.md**](slint/component-imports.md) for which globals each family uses.
4. **Export** from [`primer.slint`](slint/primer.slint) when part of the public surface.
...
**Long lists in views:** Use **`Pagination`** (`slint/Pagination/pagination.slint`), page fields on the right Slint global (`ProjectBoardListState` / `AppState` in `bridges/slint/`), and TS **`apply…SliceToWindow`** helpers for **`ArrayModel`** slices.

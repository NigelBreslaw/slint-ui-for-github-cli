...
**Implementation in this repo:** [`DialogBase/dialog-base.slint`](../DialogBase/dialog-base.slint) (**`DialogBase`**) sizes the **`PopupWindow`** to the panel (**`panel-width`** or anchor width × **`panel-height`**) and positions it with parent-relative **`x`** / **`y`** — **no** full-viewport dimmer, matching Primer **anchored** surfaces (dropdown-style). Pass **`window-inner-width`** and **`window-inner-height`** from the root **`Window`** for **horizontal clamp** and **below/above** flip. **`vertical-side`** (`auto` | `outside_bottom` | `outside_top`) and **`align`** (`start` | `center` | `end`) match a Primer subset (default anchored SelectPanel: **auto** + **start**). Enums: [`types.slint`](../DialogBase/types.slint). Panel chrome: **`OverlayTokens`** (**`backdrop-scrim`** is for modals such as [`primer-dialog.slint`](../../../app/src/ui/components/primer-dialog.slint)). Exported from [`primer.slint`](../primer.slint); gallery demo on the **Forms** page.
